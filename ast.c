...
{//对抽象语法树的先根遍历
  ...
  if (T)
	...
	case EXT_VAR_DEF:   printf("%*c外部变量定义：(%d)\n",indent,' ',T->pos);
                        ...
                        break;
    case EXT_STRU_DEF:  printf("%*c结构体结构定义：(%d)\n",indent,' ',T->pos);
	...
	case TYPE:          printf("%*c类型： %s\n",indent,' ',T->type_id);
                        break;
    case STRUTYPE:      printf("%*c类型：STRUCT\n",indent,' ');
                        printf("%*c结构体名称：\n",indent,' ');
                        display(T->ptr[0],indent+3);
                        printf("%*c结构体成员定义：\n",indent,' ');
    ...
    case VAR_DEF:       printf("%*c局部变量定义：(%d)\n",indent,' ',T->pos);
                        ...
                        break;
    case VAR_STRU:      printf("%*c访问结构体名为：\n",indent,' ');
                        display(T->ptr[0],indent+3);
                        printf("%*c访问的成员为：%s\n",indent+3,' ',T->type_id);
                        break;
    case VAR_ARR:       printf("%*c数组元素：(%d)\n",indent,' ',T->pos);
                        printf("%*c数组名称：\n",indent+3,' ');
    ...
    case DEC_LIST:      printf("%*c变量名：\n",indent,' ');
                        ...
                        while (T0) {
                            ...
                            else if (T0->ptr[0]->kind==VAR_ARR)
                                {
                                printf("%*c数组元素：(%d)\n",indent+3,' ',T->pos);
                                printf("%*c数组名称：\n",indent+6,' ');
                                display(T->ptr[0]->ptr[0],indent+9);
                                printf("%*c数组下标：\n",indent+6,' ');
                                display(T->ptr[0]->ptr[1],indent+9);
                                }
                            T0=T0->ptr[1];
                            }
                        break;
    case BREAK:         printf("%*c跳出循环体BREAK：(%d)\n",indent,' ',T->pos);
                        break;
    case CONTINUE:      printf("%*c继续下一次的循环CONTINUE：(%d)\n",indent,' ',T->pos);
                        break;
    ...
    case FOR:           printf("%*cfor循环语句：(%d)\n",indent,' ',T->pos);
                        printf("%*c循环条件：\n",indent+3,' ');
                        printf("%*cfor循环初始值：\n",indent+6,' ');
                        display(T->ptr[0],indent+9);
                        printf("%*c循环结束值：\n",indent+6,' ');
                        display(T->ptr[1],indent+9);
                        printf("%*c循环递增规则：\n",indent+6,' ');
                        display(T->ptr[2],indent+9);
                        printf("%*c循环体：\n",indent+6,' ');
                        display(T->ptr[3],indent+9);    //显示循环体
                        break;
